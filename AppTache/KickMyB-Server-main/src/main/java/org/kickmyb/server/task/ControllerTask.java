...
public class ControllerTask {
    ...
    public @ResponseBody String updateProgress(@PathVariable long taskID, @PathVariable int value) {
        ...
        ConfigHTTP.attenteArticifielle();
        MUser user = currentUser();
        serviceTask.updateProgress(taskID, value, user.id);
        return "";
    }
    ...
    public @ResponseBody String deleteOne(@PathVariable("id") long id){
        System.out.println("KICKB SERVER : Task to delete: " + id);
        ConfigHTTP.attenteArticifielle();
        MUser user = currentUser();
