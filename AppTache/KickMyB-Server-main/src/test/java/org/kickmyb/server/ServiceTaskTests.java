...
class ServiceTaskTests {
    ...
    void testSuppressionTacheIdCorrect() throws Exception {
        ...
        serviceTask.deleteOne(taskId, alice);
    ...
    void testSuppressionTacheIdIncorrect() throws Exception {
        ...
        // Tenter de supprimer une tâche avec un ID inexistant
        try {
            serviceTask.deleteOne(999L, alice);
            fail("La suppression aurait dû échouer avec un ID incorrect");
        } catch (Exception e) {
        }
        assertEquals(1, serviceTask.home(alice.id).size());
    }
    ...
    void testControleAccesSuppression() throws Exception {
        ...
        Long taskId = serviceTask.home(alice.id).get(0).id;
        try {
            serviceTask.deleteOne(taskId, bob);
            fail("Bob ne devrait pas pouvoir supprimer la tâche d'Alice");
        } catch (Exception e) {
